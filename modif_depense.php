...
// Charger l'opération en s'assurant que c'est une dépense (catégorie de type Depense)
$stmt = $mysqlClient->prepare("SELECT O.*
  FROM operation O
  JOIN categorie C ON C.ID_CATEGORIE = O.ID_CATEGORIE
  JOIN type T ON T.ID_TYPE = C.ID_TYPE
  WHERE O.ID_OPERATIONS_ = ? AND O.ID_UTILISATEUR = ? AND T.NOM_TYPE = 'Depense'");
...
<body>
  <div class="main-content">
     <header class="head"><h1>Modification de dépense</h1> 
        <div class="profil">
  ...
  <form id="ajout-revenu" method="post" action="">
    ...
    <div>
      ...
      <input type="number" id="montant" name="montant" step="0.01" value="<?= htmlspecialchars($operation['MONTANT']) ?>" placeholder="Ex: 500" required>
    </div>
    ...
    <div>
      ...
      <input class="desc" id="note" name="note" value="<?= htmlspecialchars($operation['DESCRIPTION'] ?? '') ?>" placeholder="Ex: déjeuner">
    </div>
<br> 
    <div class="conf">
      <input type="submit" value="Enregistrer" class="btn-enregistrer">
      <input type="button" value="Annuler" class="btn-annuler" onclick="window.location.href='depenses.php'">
    </div>
